...
function updateOutputs(params, model) {
  ...
  const selected = interpPoint(model.points, params.sampleTime);
  const expectedPlaques = selected.pfuPerMl * params.platedVolumeMl * params.dilutionFactor;
...
function drawPlate(model) {
  ...
  const selected = interpPoint(model.points, p.sampleTime);
  const expected = selected.pfuPerMl * p.dilutionFactor * p.platedVolumeMl;
  ...
  const maxDots = expected > 650 ? 650 : Math.min(observed, 650);
  ...
  if (expected > 1000) {
    svg += `<circle cx="260" cy="260" r="220" fill="#17243a" opacity="0.16"/>`;
    svg += `<circle cx="260" cy="260" r="190" fill="#17243a" opacity="0.08"/>`;
  }
  ...
  if (expected > 650) {
    ...
    svg += `<text x="260" y="262" text-anchor="middle" font-size="30" font-weight="800" fill="#607089" opacity="0.7">no plaques likely</text>`;
  }

  els.plateSvg.innerHTML = svg;
  const dilutionLabel = `10^${p.dilutionExponent}`;
  const observedText = expected > 650 ? `display capped at ${maxDots} visible marks` : `${observed} simulated visible plaque${observed === 1 ? '' : 's'}`;
  els.plateSummary.innerHTML = `At <strong>${formatMinutes(p.sampleTime)}</strong>, the predicted sample has <strong>${formatSci(selected.pfuPerMl)} PFU/mL</strong>. Plating <strong>${(p.platedVolumeMl * 1000).toFixed(0)} µL</strong> at <strong>${dilutionLabel}</strong> gives <strong>${formatCount(expected)}</strong> expected plaques (${status}; ${observedText}).`;
}
